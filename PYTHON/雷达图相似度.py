...
if __name__ == '__main__':
    ...
    for item1 in json_data:
        result_dic[item1["id"]] = {"companions": []}
        for item2 in json_data:
            if item2 != item1:
                result_dic[item1["id"]]["companions"].append(
                    {"companion_id": item2["id"], "similarity": compareSimilarity(item1, item2)})
        list.sort(result_dic[item1["id"]]["companions"], key=lambda x: x["similarity"])
